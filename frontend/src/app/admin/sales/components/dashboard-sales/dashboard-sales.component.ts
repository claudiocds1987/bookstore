...
export class DashboardSalesComponent implements OnInit {
  ...
  year: number = this.currentDate.getFullYear(); // año actual
  month: number = this.currentDate.getMonth() + 1; // mes actual importante + 1 porque getMonth empieza desde 0
  ...
  ngOnInit(): void {
    console.log('AÑO: ' + this.year + ' MES ACTUAL: ' + this.month);
    this.countSalesFromCurrentMonth(this.year, this.month);
    this.thisMonthRevenue(this.year, this.month);
    this.countSalesFromCurrentYear(this.year);
    this.thisYearRevenue(this.year);
  }
